...
@testable import TranslixExport
...
struct NoteTimecodeTests {
    ...
    func linksCarrySeconds() {
        let linked = NoteTimecodes.linkingTimecodes(
            in: "La fórmula [01:08] entra al parcial.", scheme: "translix-seek"
        )
        #expect(linked == "La fórmula [01:08](translix-seek://68.0) entra al parcial.")
    }

    @Test("hours are understood, since a class runs past one")
    func handlesHours() {
        let linked = NoteTimecodes.linkingTimecodes(
            in: "El repaso [1:23:45] al final.", scheme: "translix-seek"
        )
        #expect(linked.contains("translix-seek://5025.0"))
    }
