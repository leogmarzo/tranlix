...
/// Run with `TRANSLIX_INTEGRATION=1 scripts/test.sh --filter SessionPlayer`.
@Suite(
    "SessionPlayer",
    .enabled(if: ProcessInfo.processInfo.environment["TRANSLIX_INTEGRATION"] != nil)
)
@MainActor
...
private func scratch() -> URL {
    URL(filePath: NSTemporaryDirectory()).appending(path: "translix-player-\(UUID().uuidString)")
}
