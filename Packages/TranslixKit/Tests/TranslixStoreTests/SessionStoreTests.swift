...
@Suite("SessionStore renaming")
struct SessionStoreRenameTests {
    private let epoch = Date(timeIntervalSince1970: 1_754_152_200)

    @Test("renaming moves the folder, so the tree stays browsable without the app")
    func renameMovesTheFolder() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let layout = try await store.createSession(
                title: "", language: .spanish, now: epoch
            ).layout
            // Something only a move can carry across.
            try Data("ojo, esto entra al parcial".utf8).write(to: layout.userNotesURL)
            let before = try #require(store.summary(ofFolder: layout.root))

            let renamed = try await store.rename(before, to: "Clase de Estadística")

            #expect(renamed.layout.root.lastPathComponent.hasSuffix("_Clase-de-Estadística"))
            // The date still leads, which is what makes an alphabetical listing chronological.
            #expect(renamed.layout.root.lastPathComponent.hasPrefix(layout.root.lastPathComponent))
            #expect(!FileManager.default.exists(layout.root))
            #expect(FileManager.default.exists(renamed.layout.userNotesURL))
        }
    }

    @Test("the new title is on disk, not only in the folder name")
    func renameWritesTheTitle() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let layout = try await store.createSession(
                title: "Clase", language: .spanish, now: epoch
            ...
            let renamed = try await store.rename(before, to: "  Parcial de Estadística  ")

            #expect(renamed.title == "Parcial de Estadística")
            let onDisk = try SessionHandle.readManifest(at: renamed.layout.manifestURL)
            #expect(onDisk.title == "Parcial de Estadística")
        }
    }

    @Test("renaming a session to the name it already has leaves the folder where it is")
    func renameToTheSameNameKeepsTheFolder() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let layout = try await store.createSession(
                title: "Clase", language: .spanish, now: epoch
            ...
            let renamed = try await store.rename(before, to: "Clase")

            // Not "-2": the folder the name collides with is this very session.
            #expect(renamed.layout.root == before.layout.root)
            #expect(FileManager.default.exists(renamed.layout.manifestURL))
        }
    }

    @Test("taking a name another session already uses gets its own folder")
    func renameCollisionIsSuffixed() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let taken = try await store.createSession(
                title: "Clase", language: .spanish, now: epoch
            ).layout
            let mine = try await store.createSession(
                title: "Taller", language: .spanish, now: epoch
            ).layout
            let before = try #require(store.summary(ofFolder: mine.root))

            let renamed = try await store.rename(before, to: "Clase")

            #expect(renamed.layout.root.lastPathComponent.hasSuffix("-2"))
            // The recording that was already there is untouched.
            #expect(FileManager.default.exists(taken.manifestURL))
        }
    }

    @Test("a session renamed while it was busy gets its folder moved afterwards")
    func folderCatchesUpAfterTheRun() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let handle = try store.createSession(title: "", language: .spanish, now: epoch)
            let layout = await handle.layout
            let before = try #require(store.summary(ofFolder: layout.root))

            // What the app does while a recording or a transcription holds this folder open:
            // the title goes in, the folder waits.
            try await handle.setTitle("Clase de Estadística")
            #expect(FileManager.default.exists(layout.root))

            // `before` still carries the old title, so a sync that trusted the summary instead
            // of the manifest would move the folder to the wrong name — or nowhere at all.
            let synced = try store.syncFolderName(of: before)

            #expect(synced.layout.root.lastPathComponent.hasSuffix("_Clase-de-Estadística"))
            #expect(!FileManager.default.exists(layout.root))
        }
    }

    @Test("clearing the name leaves a folder that is only a timestamp")
    func renameToNothingLeavesTheStamp() async throws {
        try await withTemporaryRoot { root in
            let store = SessionStore(root: root)
            let layout = try await store.createSession(
                title: "Clase", language: .spanish, now: epoch
            ...
            let renamed = try await store.rename(before, to: "   ")

            #expect(renamed.title.isEmpty)
            let name = renamed.layout.root.lastPathComponent
