...
/// deliberately:
///
///     TRANSLIX_INTEGRATION=1 swift test --filter Integration
///
...
struct AppleSpeechEngineIntegrationTests {
    ...
    private func sample() async throws -> URL {
        let url = URL(filePath: NSTemporaryDirectory())
            .appending(path: "translix-sample-\(UUID().uuidString).caf")
...
extension ProcessInfo {
    var integrationEnabled: Bool {
        environment["TRANSLIX_INTEGRATION"] != nil
    }
}
...
struct WhisperKitIntegrationTests {
    ...
    private func sample() async throws -> URL {
        let url = URL(filePath: NSTemporaryDirectory())
            .appending(path: "translix-whisper-\(UUID().uuidString).caf")
        try await SpeechSample.write(text: spoken, languageCode: "es-MX", to: url)
        return url
    }
